...
return_statement: RETURN return_value DEL {
				...
				| {
					...
					/*if(active_func_ptr->return_type!=NONE){
							...
							printf("no return expected for function %s at line no %d",active_func_ptr->name,lineno);
						}*/
					active_func_ptr=&global;
		...
		| UNARY_OP factor {
			...
			if($2->type!=STRING1){
				if($2->type!=BOOL1){
					...
					sem_flag=1;
					cout<<"unsuported operand with bool at line no: "<<lineno<<endl;
					$$->type=ERROR;
				}
			}
			else{
